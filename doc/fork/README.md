...
**ビルド手順はこのフォークで未確認である。**
`blueMSX/Make/` に MSVC 2022 / MSVC 2026 / GNU make の 3 系統がある。
確かめたら `build/README.md` に書く。
...
**blueMSX+ を起動するとウィンドウが出る。** コマンドラインにヘッドレスで
走らせる手段は見当たらない（**確認済み(読解)**: `Src/Emulator/CommandLine.c` の
オプション表に、スクリプト実行も自動終了も無い）。
起動すると人間の並行作業と衝突しうるので、自動で繰り返し起動しない。
