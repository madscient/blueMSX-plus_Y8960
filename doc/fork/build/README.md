...
### 3.4 ファイルを書き戻しても再コンパイルされないことがある

**症状**: ソースを元に戻してビルドし直したのに、**壊したままの動作が続く**。
`git diff` は空で、ビルドも成功する。

**原因**: PowerShell の `Copy-Item` は**元ファイルの更新時刻を引き継ぐ**。
退避しておいたファイルを書き戻すと、ソースの時刻が退避した時点まで巻き戻り、
中間ファイル（`.obj`）より古くなる。msbuild は「更新不要」と判断して
**再コンパイルを飛ばす**。リンクは走るので、ビルド自体は成功する。

**確認のしかた**: ソースと `.obj` の更新時刻を見る。
`.obj` の方が新しければ、そのソースの変更は入っていない。

```sh
ls -la --time-style=full-iso blueMSX/Src/Memory/<file>.c
ls -la --time-style=full-iso blueMSX/Make/msvc2022/x64/Release/<file>.obj
```

**対処**: 書き戻したら更新時刻を現在にする。

```powershell
(Get-Item "blueMSX\Src\Memory\<file>.c").LastWriteTime = Get-Date
```
...
**枝を切り替えたら、試す前にその枝でビルドし直す。** 出力先 `x64/Release` は枝で共有なので、
最後にビルドした枝の実行ファイルが残る。別の枝の実行ファイルでは、その枝にしか無い
ROM タイプを使う機種が `unknown machine` になる（**確認済み**）。
